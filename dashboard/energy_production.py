...
def energy_production():
    ...
    st.header("Growth of Solar in the Energy Mix of Benelux")
    ...
    energy_production_benelux['Value'] = energy_production_benelux['Value'].astype(float)

    # Filter for the last five years without 2023
    current_year = datetime.now().year
    last_five_years = list(range(current_year - 10, current_year))
    benelux = energy_production_benelux[energy_production_benelux['Country'].isin(['Belgium', 'Netherlands', 'Luxembourg'])]
    benelux = benelux[benelux['Time'].apply(lambda x: pd.to_datetime(x).year).isin(last_five_years)]

    # Create t1 DataFrame
    t1 = benelux[benelux['Product'] == 'Solar']
    t1 = t1.groupby(['Country', t1['Time'].apply(lambda x: pd.to_datetime(x).year)]).agg({'Value': 'sum'}).rename(columns={'Value': 'GWh Solar'}).reset_index()

    # Create t2 DataFrame
    t2 = benelux[(benelux['Product'] != 'Total Renewables (Hydro, Geo, Solar, Wind, Other)') & (benelux['Product'] != 'Total Combustible Fuels')]
    t2 = t2.groupby(['Country', t2['Time'].apply(lambda x: pd.to_datetime(x).year)]).agg({'Value': 'sum'}).rename(columns={'Value': 'GWh Total'}).reset_index()

    # Merge t1 and t2 DataFrames
    merged_df = pd.merge(t1, t2, on=['Country', 'Time'])

    # Calculate Solar Share (%)
    merged_df['Solar Share (%)'] = round((merged_df['GWh Solar'] / merged_df['GWh Total']) * 100, 2)

    # Rename the 'Time' column to 'Year'
    merged_df.rename(columns={'Time': 'Year'}, inplace=True)

    # Select necessary columns
    result_df = merged_df[['Country', 'Year', 'GWh Solar', 'GWh Total', 'Solar Share (%)']]
